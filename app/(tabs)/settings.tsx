...
export default function TabTwoScreen() {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={{ paddingTop: convert(16) }}>
...
const History = () => {
  const { articles, error, loading } = useArticlesAll()
    ...
    <ScrollView horizontal contentContainerStyle={{ paddingHorizontal: convert(16), gap: convert(12) }}>
      {
        articles?.map((article, index) => (
          <HistoryItems key={index} article={article} />
        ))
      }
...
const HistoryItems = ({ article }: { article: Articles }) => {
  return (
    <View style={{ width: convert(150) }}>
      <Image source={{ uri: `https://${article.imageurl}` }} style={{ width: convert(150), marginBottom: convert(8), aspectRatio: 1, borderWidth: 1, borderColor: '#eee' }} />
      <View style={{ gap: convert(8) }}>
        <Text style={{ color: '#777', fontSize: convert(11), fontWeight: 'bold', textTransform: "uppercase" }}>{moment(article.createdAt).fromNow()}</Text>
        <Text style={{ fontSize: convert(16), fontWeight: 'semibold' }}>{article.title.length > 48 ? article.title.substring(0, 48) + '...' : article.title}</Text>
      </View>
    </View>
  )
}
