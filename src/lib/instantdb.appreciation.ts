...
export const removeAppreciation = async (id: string) => {
    try {
        if (__DEV__) console.log("[removeAppreciation] ID", id)
        const appreciation = await db.transact(db.tx.appreciation[id].delete())
        if (__DEV__) console.log("[removeAppreciation] Appreciation Removed", appreciation)
        return appreciation.clientId
    } catch (error) {
        if (__DEV__) console.log("[removeAppreciation] Error", error)
    }
}
...
export const getAppreciationStateForUser = async (userId: string, articleId: string) => {
    try {
        ...
        if (__DEV__) console.log("[getAppreciationStateForUser] Error", error)
    }
