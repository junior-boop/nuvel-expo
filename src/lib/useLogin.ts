...
export default function useLogin({ name, first_name, email }: { name: string, first_name: string, email: string }) {
    ...
    const getuserinfo = useCallback(async (data: Partial<User>) => {
        ...
        try {
            ...
            if (__DEV__) console.log('[useLogin] ✅ Session créée, userData:', result.user);
            ...
            if (result.user.photo === null || result.user.biography === null || result.user.biography === '') {
                if (__DEV__) console.log('[useLogin] 📝 Profil incomplet, redirection vers /loginzone/usersinfos');
                router.replace({
                ...
                if (__DEV__) console.log('[useLogin] ✅ Profil complet, redirection vers /(tabs)');
                router.replace({
                    pathname: "/(tabs)",
                    params
                })
