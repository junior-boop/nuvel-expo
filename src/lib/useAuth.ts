...
export const useAuth = (
  ...
  const loadUserFromStorage = async () => {
    try {
      ...
      if (userData) {
        ...
        if (__DEV__) console.log('[Auth] Utilisateur chargé depuis le stockage');
      }
    } catch (err) {
  ...
  const saveUserToStorage = async (userData: User) => {
    try {
      ...
      if (__DEV__) console.log('[Auth] Utilisateur sauvegardé');
    } catch (err) {
  ...
  const signIn = useCallback(async (userData: SignInData): Promise<boolean> => {
    try {
      setLoading(true);
      setError(null);
      if (__DEV__) console.log('[Auth] Connexion...', userData.email);
      ...
      if (result.data) {
        ...
        if (__DEV__) console.log('[Auth] ✅ Connexion réussie:', result.message);
        return true;
      } else {
  ...
  const refreshUser = useCallback(async () => {
    ...
    try {
      ...
      if (__DEV__) console.log('[Auth] Utilisateur rafraîchi');
    } catch (err) {
