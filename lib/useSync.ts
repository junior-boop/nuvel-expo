...
export const useSync = (
    ...
    const getDeviceId = useCallback(async (): Promise<string> => {
        ...
        if (!deviceId) {
            ...
            console.log('[Sync] 🆔 Nouveau device ID créé:', deviceId);
        }
        return deviceId;
    }, []);
    ...
    const queueChange = useCallback(async (change: SyncChange) => {
        try {
            ...
            console.log('[Sync] ✅ Changement ajouté à la queue:', change.entityType, change.action);
        } catch (error) {
    ...
    const sync = useCallback(async (): Promise<SyncResult> => {
        try {
            ...
            console.log('[Sync] 📦 Changements à envoyer:', changes.length);
            ...
            if (result.success) {
                ...
                if (result.serverChanges && result.serverChanges.length > 0) {
                    console.log('[Sync] 📥 Changements serveur à appliquer:', result.serverChanges.length);
    ...
    const applyServerChange = useCallback(async (change: SyncChange) => {
        try {
            console.log('[Sync] 📥 Application changement serveur:', change.entityType, change.action);
            ...
            console.log('[Sync] ✅ Changement appliqué:', change.entityId);
        } catch (error) {
