...
async function initializeTokens() {
    ...
    console.log('[TokenSystem] Tokens initialized:', {
...
async function refreshAccessToken() {
    try {
        if (!refreshToken) {
            console.log('[TokenSystem] No refresh token available');
            return null;
        }
        ...
        console.log('[TokenSystem] ✅ Access token refreshed');
        return accessToken;
    } catch (error) {
...
export async function setTokens(access: string, refresh: string) {
    ...
    console.log('[TokenSystem] ✅ Tokens set');
}
...
export async function clearTokens() {
    ...
    console.log('[TokenSystem] 🗑️ Tokens cleared');
}
    ...
    const checkInterval = setInterval(async () => {
        ...
        if (currentAccessToken) {
            ...
            if (timeUntilExpiry < 60000 && timeUntilExpiry > 0) {
                console.log('[TokenSystem] Token expiring soon, refreshing...');
                await refreshAccessToken();
            }
        } else {
            // await refreshAccessToken();
            console.log('[TokenSystem] No access token available', currentAccessToken);
        }
